export async function ParserStart(
	...
	try {
		const response = await fetch(
			`https://api-dev.autostructure.ru/v1/new_parser/start/${filterId}?using_proxy=${currentProxySource}`,
			{
				method: 'GET',
				headers: {
...
export async function ParserStop(
	...
	try {
		const response = await fetch(
			`https://api-dev.autostructure.ru/v1/new_parser/stop`,
			{
				method: 'GET',
				headers: {
					'Content-Type': 'application/json',
...
export async function GetBrightProxyTrafficAvalibale(
	...
	try {
		const response = await fetch(
			'https://api-dev.autostructure.ru/v1/proxies/get_balance_bright_data',
			{
				method: 'GET',
			}
		)
