...
export async function ParserStop(
	...
	try {
		...
		return {
			success: true,
			message: language === 'RU' ? 'Парсер остановлен' : 'Parser stopped',
		}
	} catch {
		return {
			success: false,
...
export async function GetBrightProxyTrafficAvailable(
	...
	try {
		const data = await fetchAPI<{ balance: number; pending_costs: number }>(
			'/proxies/get_balance_bright_data'
		)
		return {
			success: true,
			balance: data.balance,
			pendingCosts: data.pending_costs,
		}
	} catch {
		return {
			success: false,
