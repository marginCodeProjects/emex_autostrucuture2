...
const AllDataTable: React.FC<AllDataTableProps> = ({ setFileId }) => {
    ...
    const afterParsingPopup = (id: number) => {
        return (
            <div>
                <a href={`https://127.0.0.1:8000/v1/files/download_file/after_parsing/${id}`} className={`${styles.table__textsForLinks} ${styles.inter__medium}`} >Скачать файл</a>
                <p className={`${styles.table__texts} ${styles.inter__medium}`} style={{ width: '100%' }} onClick={() => setFileId(id)}>Просмотреть файл</p>
            </div>
        );
    }
    return (
                ...
                <div className={styles.table__container}>
                    ...
                    {files?.map((file) => (
                        <div key={file.id} className={styles.table__line}>
                            ...
                            <Popover content={afterParsingPopup(file.id)} >
                                <p className={`${styles.table__textsForLinks} ${styles.inter__medium}`} >{file.after_parsing_filename}</p>
                            </Popover>
                        </div>
                    ))}
                </div>
