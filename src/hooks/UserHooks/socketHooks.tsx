...
const useWebSocket = () => {
    ...
    const { token } = useAuth();
    useEffect(() => {
        const wsStatus = new WebSocket(`wss://api.autostructure.ru127.0.0.1:8000/v1/new_parser/websocket_status/${token}`);
        ...
        const wsPercent = new WebSocket(`wss://api.autostructure.ru127.0.0.1:8000/v1/new_parser/websocket_percent/${token}`);
