...
const useWebSocket = () => {
    ...
    const { token } = useAuth();
    useEffect(() => {
        const wsStatus = new WebSocket(`wss://api-dev.autostructure.ru/v1/new_parser/websocket_status/${token}`);
        ...
        const wsPercent = new WebSocket(`wss://api-dev.autostructure.ru/v1/new_parser/websocket_percent/${token}`);
